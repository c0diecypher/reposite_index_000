...
Zipper App снова ждет ваших заказов! ⚡`;
            ...
            if (result && result.link && result.id) {
              ...
              if (resGetPayment && resGetPayment.status !== undefined) {
                const status = resGetPayment.status;
                console.log('Статус оплаты:', status);
              } else {
                console.log('Статус оплаты не найден или равен undefined.');
              }
              // Создаем URL для второго запроса
              ...
              console.log('status:',getPaymentStatus);
              // Отправляем второй POST-запрос
...
app.get('/get/payment', async (req, res) => {
    const status = JSON.parse(resGetPayment)
 res.json({ status });
});
